...
The above packages expect input data as obtained, e.g., from RNA-seq or another
...
with Bioconductor differential gene expression packages. 
 ...
 - This approach corrects for any potential
changes in gene length across samples (e.g. from differential isoform
usage) [@Trapnell2013Differential]; 
 - Some of these methods are substantially faster and require less memory and disk usage compared
...
sequence [@Robert2015Errors]. 
- Note that transcript abundance quantifiers skip the generation of large files which store read alignments (SAM or BAM files), instead producing smaller files which
store estimated abundances, counts and effective lengths per
transcript. 
- For more details, see the manuscript describing this approach
...
## Install *Salmon* in a conda environment called *salmon*

Run this command in the `Linux Terminal` from RStudio or a Bash Terminal
...
## Downloading the Version 27 human transcripts
```{r, eval=FALSE}
url <- "ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_27/gencode.v27.transcripts.fa.gz"

dir.create(file.path(root, "temp"))
download.file(url = url, 
              destfile = file.path(root,
                                  "temp",
                                  "gencode.v27.transcripts.fa.gz"), cacheOK = FALSE)
```
...
```
salmon index -i gencode.v27_salmon_0.10.0 -t gencode.v27.transcripts.fa.gz
```

The `0.10.0` refers to the version of *Salmon* that was used. As of writing,
*Salmon* is up to version 0.10.1 and it's always best to use the
latest version of software available.

To download a file containing the raw read of one sample we can use the `SRA` toolkit from NCBI. We can install the toolkit using

```
conda install -c bioconda sra-tools
```

To download one sample (we use the first sample id from the experiment 
```{r}
# devtools::install_github("mikelove/airway2")
library("airway2")
dir <- system.file("extdata", package="airway2")
dir
list.files(dir)
list.files(file.path(dir,"quants"))[1] 

```

We can download the raw reads from this `r list.files(file.path(dir,"quants"))[1]` sample by using the SRA toolkit in the command  line

```
prefetch -v SRR1039508
```

To analyse this sample we need to convert it to fastq and rename to 
```
fastq-dump --outdir /home/<user>/ncbi/public/sra/fastq --split-files /home/marc_teunis/ncbi/public/sra/SRR1039508.sra
```


## Calculating transcript abundance for one sample of "airway2"

To quantify an individual sample, `SSR039508`, the following command
can be used:
**NOTE:** Mkae sure the gencode.v27 index file is in the same directory as the downloaded raw reads (fastq format). Naviagte into this folder in the Terminal and run the command below. **this takes about 5-10 minutes on an average laptop.**
```
salmon quant -i gencode.v27_salmon_0.10.0 -p 6 --libType A \
  --gcBias --biasSpeedSamp 5 \
  -1 SRR1039508_1.fastq -2 SRR1039508_2.fastq \
  -o sample_01
```
...
using GC bias correction (the bias speed part is no longer
...
table(DESeq2 = res$padj < 0.1)
table(edgeR = tt.all$table$FDR < 0.1)
```
...
log2 fold-change threshold to 1 (i.e., fold change threshold equal to 2), as for
...
table(DESeq2 = resLFC1$padj < 0.1)
table(edgeR = tt.treat$table$FDR < 0.1)
```
