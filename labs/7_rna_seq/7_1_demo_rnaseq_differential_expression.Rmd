...
subtitle: "Bioconductor Course ILC" 
author: "Adaptations - Marc Teunis"
date: "`r Sys.Date()`"
output:
...
# Disclaimer;
 ...
 - This workflow contains only reference to DESeq2 Differential Analysis Expression analysis. I chose to remove reference to other workflows for clarity. 
 - The starting point for this workflow is an existing or constructed SummarizedExperiment formal R-Class object.
 - In order to contruct a `SummarizedExperiment` you need at least three things:
 
 1. The matrix containing raw counts per feature (in rows) and the samples (in columns)
 2. The columdata describing the samples and experimental design
 3. The rowdata describing the features (annotations)

To construct a SummarizedExperiment you can use the constructor function `SummarizedExperiment()` 
...
## Abstract
 ...
 - Start from the FASTQ files
 - Show how these were quantified with respect to a reference transcriptome, 
 - Prepare a count matrix which tallies the number of RNA-seq fragments mapped to each gene for each sample. 
 - Perform exploratory data analysis (EDA) for quality assessment and to
explore the relationship between samples, 
 - Perform differential gene expression analysis (DGE) 
 - Visually explore the results.

## Introduction

 - Bioconductor has many packages which support analysis of
high-throughput sequence data, including RNA sequencing (RNA-seq)
...
and working with gene annotations [@Huber2015Orchestrating] 
- Also contributed packages for statistical analysis and
...
## Experimental data
 ...
 - Stored in an R package, *airway2*
...
[@Himes2014RNASeq]. 
 - Glucocorticoids are used, for example, 
by people with asthma to reduce inflammation of the airways. 
 - Four primary human airway smooth muscle cell lines were treated with 1
micromolar dexamethasone for 18 hours. 
- Each of the four cell lines have a treated and an untreated sample. 
For more description
of the experiment see the [PubMed entry 24926665](http://www.ncbi.nlm.nih.gov/pubmed/24926665)

## Raw Data

For raw data see the
...
The data can also be accessed under accession number `GSE52778`.
We use this later to access the expression set and construct a SummarizedExperiment formal R-Class object.
...
The above packages expect input data as obtained, e.g., from RNA-seq or another high-throughput sequencing experiment, in the form of a matrix of counts.

This matrix takes the general form:
```{r}
count_mtrx <- tribble(~"sample_1", ~"sample_2", ~"sample_3",
                      122,        33,         0,
                      2,          223,        333) %>%
  as.matrix()
rownames(count_mtrx) <- c("ENS_id_1", "ENS_id_2")
count_mtrx
```

## Statistics on counts

 - Values in the matrix should be counts of sequencing
reads/fragments. 
 - Important for the statistical models used by
*DESeq2* and *edgeR* to hold, as only counts allow assessing the
measurement precision correctly. 
 - Not use counts that were pre-normalized for sequencing depth/library size, as
the statistical model is most powerful when applied to un-normalized
counts
- Is designed to account for library size differences
internally.
...
library(tidyverse)
coldata <- read_delim(file.path(dir, "SraRunTable.txt"), delim = "\t")
colnames(coldata)
idx <- c("Run","cell_line","treatment")

coldata <- coldata %>%
  dplyr::select(idx) %>%
  dplyr::rename(dex = treatment,
                cell = cell_line)

coldata
# coldata[,idx] base R solution
```
...
```{r}
#coldata <- coldata[,idx]
colnames(coldata) <- colnames(coldata) %>% tolower()
coldata$cell
coldata$dex
```
...
coldata <- coldata %>%
  mutate(dex = 
  forcats::fct_recode(as_factor(dex), 
                      "untrt" = "Untreated",     ## see note below
                      "trt" = "Dexamethasone")
)
...
*relevel* the `dex` factor in one go with `forcats_recode`. The order in which we change the factor levels will be the new order.
...
```{r}
tx2gene <- read_csv(file.path(dir, "gencode.v27.annotation.tx2gene.csv.gz"))
head(tx2gene)
```
...
For clarity we only show the DESeq2 workflow. Below you can find a comparison.

[compared performance](https://www.ncbi.nlm.nih.gov/pmc/articles/pmid/27022035/) 
of different statistical methods for RNA-seq using a large number of biological replicates and can help to decide which tools make sense to use, and how many
...
The *DESeq2* package will store the data and intermediate results in an object called a *DESeqDataSet*. This object builds on top of a general Bioconductor class of object called the
...
The *SummarizedExperiment* container is diagrammed in the Figure above and discussed in the latest Bioconductor paper [@Huber2015Orchestrating].
...
accessed with `assays`. The `rowData` for our object will keep track of the intermediate calculations for each gene: e.g. the mean of normalized counts, the dispersion estimates, the estimated
coefficients and their standard error, etc. We can also include
information about the location of each gene, in which case, we would use `rowRanges` to access the *GenomicRanges* or *GenomicRangesList* associated with each gene.The component parts of the *SummarizedExperiment* are accessed with an R function of the same name: `assay` (or `assays`), `rowRanges`/`rowData` and `colData`. 
...
Bioconductor software packages often define and use a custom class for storing data that makes sure that all the needed data slots are
...
packages. Additionally, the core Bioconductor classes provide useful functionality: for example, subsetting or reordering the rows or columns of a *SummarizedExperiment* automatically subsets or reorders the associated *rowData* and *colData*, which can help to prevent accidental sample swaps that would otherwise lead to spurious results. With *SummarizedExperiment* this is all taken care of behind the scenes.

## DESeqDataSet

 - In *DESeq2*, the custom class is called *DESeqDataSet*. 
 - Main differences with *SummarizedExperiment* is that we will use the
*counts* function to access the `assay` matrix, 
- It is required that this matrix stores non-negative integers (this is to prevent mistaken use of the package for non-count data).
...
*design formula*. 
- The designformula tells which columns in the sample information table (`colData`)
...
post <- nrow(dds)
(post/pre)*100 # percentage reduction 
```
...
Note that the two transformations offered by DESeq2 are provided for applications *other* than differential testing. 
For differential testing we recommend the
*DESeq* function applied to raw counts, as described later
in this workflow, which also takes into account the dependence of the variance of counts on the mean value during the dispersion estimation step.
...
contain treatment type and cell type instead of sample ID, so
...
different plotting shapes) are considerable, though not stronger than the differences due to treatment with dexamethasone (red vs blue color). This shows why it will be important to account for this in differential testing by using a paired design
...
normalized counts. The new procedure results in a gain of almost 70 genes.

```{r}
(table(IHW=resIHW$padj < .1) %>% as_tibble())[2,2] -
(table(old=res$padj < .1) %>% as_tibble())[2,2] 
```
...
readr::write_excel_csv(resOrderedDF[1:100, ], path = file.path(
  root, 
  "data",
  "rnaseq_results.csv"))
```

A more sophisticated way for exporting results can be done with the 
Bioconductor package `r Biocpkg("ReportingTools")` [@Huntley2013ReportingTools].
...
We can plot the factors estimate  d by *RUV*:
...
Love MI, Anders S, Kim V and Huber W. RNA-Seq workflow: gene-level exploratory analysis and differential expression [version 2; referees: 2 approved]. F1000Research 2016, 4:1070 (https://doi.org/10.12688/f1000research.7035.2) 
