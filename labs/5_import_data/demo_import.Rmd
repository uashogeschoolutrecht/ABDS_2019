 ...
 * You could point to a file with an absolute path: 
 The file for the "dengue_data" dataset in my RStudio Server Home folder is located at:
```{r, getting_absolute}
wd <- getwd()
file_path <- file.path(wd, "data", "dengue_data.csv")
file_path
```
...
```{r, file_path_example}
path_to_dengue <- file.path(here::here("data", 
                                       "dengue_data.csv"))

dengue <- read_csv(path_to_dengue, skip = 11)
```

## Using the Linux Terminal
Remember the Linux Terminal:
Open a new terminal and get the `head` (first 20 lines) of the `"data/dengue_data.csv"` file

```{bash, eval = FALSE}
head -20 data/dengue_data.csv
```
Run this command above in a Lnux Terminal. Why did I set skip = 20 in the R-code above?
