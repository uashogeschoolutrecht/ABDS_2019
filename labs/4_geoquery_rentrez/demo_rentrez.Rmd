---
title: "Exercise - Rentrez, an R package to interface with the NCBI databases"
...
On the basis of https://cran.r-project.org/web/packages/rentrez/vignettes/rentrez_tutorial.html#introduction-the-ncbi-entrez-and-rentrez. and the "rentrez" 
Vignette by David Winter.
...
```{r eg_search}
r_search_r <- entrez_search(db="pubmed", term="R Language")
r_search_teunis <- entrez_search(db="pubmed", term="Teunis M.A.")
```
...
Once again, the object returned by `entrez_summary` behaves like a list, so you can extract elements using `$` or `[[i]]`. For instance, we could convert our PubMed ID to another article identifier...
